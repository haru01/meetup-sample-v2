...
test.describe("認証", () => {
  test.describe("新規登録", () => {
    test("新規アカウントを登録するとホームにリダイレクトされる", async ({
      page,
    }) => {
  ...
  test.describe("ログイン", () => {
    test("登録済みアカウントでログインするとヘッダーにユーザー名が表示される", async ({
      page,
    }) => {
      const suffix = uniqueSuffix();
    ...
    test("誤った認証情報ではログインに失敗しユーザーは認証されない", async ({
      page,
    }) => {
      await page.goto("/login");
  ...
  test.describe("ログアウト", () => {
    test("ログアウトするとログインページにリダイレクトされる", async ({
      ...
      await page
        .getByLabel("メールアドレス")
        .fill(`logout${suffix}@example.com`);
      await page.getByLabel("パスワード").fill("password123");
      await page.getByRole("button", { name: "登録" }).click();
      await expect(page).toHaveURL("/");
