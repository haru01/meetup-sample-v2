...
async function createCommunityAndEvent(
  ...
  await expect(
    page.getByRole("heading", { name: `テストコミュニティ${suffix}` }),
  ).toBeVisible();
...
async function approveAll(page: Page, eventId: string): Promise<void> {
  ...
  await expect(
    page.getByRole("heading", { name: "申し込み一覧" }),
  ).toBeVisible();
...
test.describe("ハッピーパス", () => {
  test("企画→公開→申し込み→承認→チェックイン→クローズ", async ({ browser }) => {
    ...
    await expect(
      organizer.getByText("イベントをクローズしました"),
    ).toBeVisible();
...
test.describe("シナリオB: 参加者キャンセル", () => {
  test("参加確定後にキャンセルするとキャンセル済みになる", async ({
    browser,
  }) => {
    const suffix = uniqueSuffix();
...
test.describe("シナリオC: イベント中止", () => {
  test("公開済みイベントを中止するとステータスが CANCELLED になる", async ({
    browser,
  }) => {
    const suffix = uniqueSuffix();
